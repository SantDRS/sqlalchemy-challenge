...
def precipitation():
    ...
    """List of precipitation(prcp) and date(date) data"""
...
def tobs():
    ...
    """List of dates and temps for most active station for the last year of data""" 
    # Create query to find the last date in the database
    
    last_year_query_results = session.query(measurement.date).order_by(measurement.date.desc()).first() 
    ...
    active_station= session.query(Measurement.station, func.count(Measurement.station)).order_by(func.count(Measurement.station).desc()).group_by(Measurement.station).first()
    ...
    dates_tobs_last_year_query_results = session.query(Measurement.date, Measurement.tobs, Measurement.station).filter(Measurement.date > query_start_date).filter(Measurement.station == most_active_station) 
...
def start_date(start):
    ...
    """List of the min temp, the avg temp, and max temp for given start date."""
...
def Start_end_date(start, end):
    ...
    """List of min, max and avg tobs between start and end dates entered"""
    ...
    start_end_date_tobs_results = session.query(func.min(measurement.tobs), func.avg(measurement.tobs), func.max(measurement.tobs)).filter(measurement.date >= start).\
